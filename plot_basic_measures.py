...
if __name__ == '__main__':
    ...
    fig = nf.plot_multiple_distributions_with_colorbar(datavecs=degrees.values(), labels=cities, xlabel='k',
                                                       ylabel='P(k)', c=areas, max_k=28)
    ...
    for cluster, list_cities in clusters.items():
        ...
        fig = nf.plot_multiple_distributions_with_colorbar(datavecs=datavecs, labels=list_cities, xlabel='k',
                                                           ylabel='P(k)', c=areas, max_k=max_k)
    ...
    for city, list_degrees in degrees.items():
        plt.close('all')
        fig, m = nf.plot_degree_distribution(degrees=list_degrees, city=city, xlabel='k', ylabel='P(k)',
                                             max_k=int(df_degrees[city][1]))
